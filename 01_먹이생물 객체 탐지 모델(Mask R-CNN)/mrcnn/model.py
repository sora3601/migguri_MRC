...
############################################################
#  seulki+chatGPT 모델 저장 콜백함수 만들기
############################################################
class LossLoggingCallback(keras.callbacks.Callback):
    def __init__(self, config_name, log_dir, num_models_to_save):
        super().__init__()
        self.best_losses = []
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # now epoch
        model_file = os.path.join(self.log_dir, f"mask_rcnn_{self.config_name}_{(epoch+1):04d}.h5")
        loss = logs['loss']
        
        self.best_losses.append((model_file, loss))
        
        print('\n' + '☆'*50)
        print('self.best_losses:', self.best_losses)
        print('☆'*50 + '\n')
        
        if len(self.best_losses) > self.num_models_to_save:
            print('\n' + '★'*50)
            self.best_losses.sort(key=lambda x: x[1]) # loss 기준 정렬
            
            print('self.best_losses:', self.best_losses)
            
            rm_model, max_loss = self.best_losses.pop()
            
            print('삭제할 모델:', rm_model)
            os.remove(rm_model)
            print('★'*50 + '\n')
...
class MaskRCNN(object):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            # ★seulki, Loss 값을 저장하는 콜백 추가
            keras.callbacks.CSVLogger(filename=os.path.join(self.log_dir, 'training_log.csv'), separator=',', append=True),
            
            LossLoggingCallback(config_name=self.config.NAME.lower(), log_dir=self.log_dir, num_models_to_save=3),
        ...
        print('learning_rate:',learning_rate, "/ learning_mom",self.config.LEARNING_MOMENTUM)
